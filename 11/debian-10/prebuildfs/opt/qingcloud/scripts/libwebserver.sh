#!/bin/bash
#
# Qingcloud web server handler library
...
web_server_execute() {
    ...
    (
        . "/opt/qingcloud/scripts/lib${web_server}.sh"
        . "/opt/qingcloud/scripts/${web_server}-env.sh"
        "$@"
    )
}
...
web_server_list() {
    ...
    for web_server in "${supported_web_servers[@]}"; do
        [[ -f "/opt/qingcloud/scripts/${web_server}-env.sh" ]] && existing_web_servers+=("$web_server")
    done
...
web_server_validate() {
    ...
    if [[ -z "$(web_server_type)" || ! " ${supported_web_servers[*]} " == *" $(web_server_type) "* ]]; then
        ...
        print_validation_error "Could not load the $(web_server_type) web server library from /opt/qingcloud/scripts. Check that it exists and is readable."
    fi
