...
```console
$ curl -sSL https://raw.githubusercontent.com/qingcloud/qingcloud-docker-pgpool/master/docker-compose.yml > docker-compose.yml
$ docker-compose up -d
```

You can find the default credentials and available configuration options in the [Environment Variables](#environment-variables) section.

# Why use Qingcloud Images?

* Qingcloud closely tracks upstream source changes and promptly publishes new versions of this image using our automated systems.
* With Qingcloud images the latest bug fixes and features are available as soon as possible.
* Qingcloud containers, virtual machines and cloud images use the same components and configuration approach - making it easy to switch between formats based on your project needs.
* All our images are based on [minideb](https://github.com/qingcloud/minideb) a minimalist Debian based container image which gives you a small base container image and the familiarity of a leading Linux distribution.
* All Qingcloud images available in Docker Hub are signed with [Docker Content Trust (DCT)](https://docs.docker.com/engine/security/trust/content_trust/). You can use `DOCKER_CONTENT_TRUST=1` to verify the integrity of the images.
* Qingcloud container images are released daily with the latest distribution packages available.

> This [CVE scan report](https://quay.io/repository/qingcloud/pgpool?tab=tags) contains a security report with all open CVEs. To get the list of actionable security issues, find the "latest" tag, click the vulnerability report link under the corresponding "Security scan" field and then select the "Only show fixable" filter on the next page.
...
Non-root container images add an extra layer of security and are generally recommended for production environments. However, because they run as a non-root user, privileged tasks are typically off-limits. Learn more about non-root containers [in our docs](https://docs.qingcloud.com/tutorials/work-with-non-root-containers/).

# Supported tags and respective `Dockerfile` links

Learn more about the Qingcloud tagging policy and the difference between rolling tags and immutable tags [in our documentation page](https://docs.qingcloud.com/tutorials/understand-rolling-tags-containers/).


* [`4`, `4-debian-10`, `4.2.2`, `4.2.2-debian-10-r72`, `latest` (4/debian-10/Dockerfile)](https://github.com/qingcloud/qingcloud-docker-pgpool/blob/4.2.2-debian-10-r72/4/debian-10/Dockerfile)

Subscribe to project updates by watching the [qingcloud/pgpool GitHub repo](https://github.com/qingcloud/qingcloud-docker-pgpool).

# Get this image

The recommended way to get the Qingcloud Pgpool-II Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/qingcloud/pgpool).

```console
$ docker pull qingcloud/pgpool:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/qingcloud/pgpool/tags/) in the Docker Hub Registry.

```console
$ docker pull qingcloud/pgpool:[TAG]
```

If you wish, you can also build the image yourself.

```console
$ docker build -t qingcloud/pgpool:latest 'https://github.com/qingcloud/qingcloud-docker-pgpool.git#master:4/debian-10'
```
...
$ docker run --detach --rm --name pg-0 \
  ...
  qingcloud/postgresql-repmgr:latest
...
services:
  pg-0:
    image: qingcloud/postgresql-repmgr:11
    ports:
      - 5432
    volumes:
  ...
  pgpool:
    ...
    healthcheck:
      test: ["CMD", "/opt/qingcloud/scripts/pgpool/healthcheck.sh"]
      interval: 10s
      timeout: 5s
      retries: 5
  myapp:
...
A HA PostgreSQL cluster with Pgpool, [Streaming replication](https://www.postgresql.org/docs/10/warm-standby.html#STREAMING-REPLICATION) and [repmgr](https://repmgr.org) can easily be setup with the Qingcloud PostgreSQL with Replication Manager and Pgpool Docker Images using the following environment variables:
...
With Docker Compose the HA PostgreSQL cluster can be setup using the [`docker-compose.yml`](https://github.com/qingcloud/qingcloud-docker-pgpool/blob/master/docker-compose.yml) file present in this repository:
...
1. Using `docker run`
    ...
    ```console
    $ docker run \
        -v /path/to/certs:/opt/qingcloud/pgpool/certs \
        -e ALLOW_EMPTY_PASSWORD=yes \
        -e PGPOOL_ENABLE_TLS=yes \
        -e PGPOOL_TLS_CERT_FILE=/opt/qingcloud/pgpool/certs/postgres.crt \
        -e PGPOOL_TLS_KEY_FILE=/opt/qingcloud/pgpool/certs/postgres.key \
        qingcloud/pgpool:latest
    ```
    ...
    services:
      ...
      ...
        environment:
          ...
          - PGPOOL_TLS_CERT_FILE=/opt/qingcloud/pgpool/certs/postgres.crt
          - PGPOOL_TLS_KEY_FILE=/opt/qingcloud/pgpool/certs/postgres.key
        ...
        volumes:
          ...
          - /path/to/certs:/opt/qingcloud/pgpool/certs
      ...
    ```

Alternatively, you may also provide this configuration in your [custom](https://github.com/qingcloud/qingcloud-docker-pgpool#configuration-file) configuration file.
...
Please see the list of environment variables available in the Qingcloud Pgpool container in the next table:
...
The Qingcloud Pgpool-II Docker image sends the container logs to `stdout`. To view the logs:
...
Qingcloud provides up-to-date versions of Pgpool-II, including security patches, soon after they are made upstream. We recommend that you follow these steps to upgrade your container.
...
We'd love for you to contribute to this container. You can request new features by creating an [issue](https://github.com/qingcloud/qingcloud-docker-pgpool/issues), or submit a [pull request](https://github.com/qingcloud/qingcloud-docker-pgpool/pulls) with your contribution.

# Issues

If you encountered a problem running this container, you can file an [issue](https://github.com/qingcloud/qingcloud-docker-pgpool/issues/new). For us to provide better support, be sure to include the following information in your issue:
